...
export class Attack {
    ...
    isReady(): boolean {
        return this.entity.world.state.tick > this.lastUsed + (this.weapon?.attackCooldown || 20);
    }
    ...
    performAttack(defender: Entity) {
        ...
        const targetAC = defender.finalStats.DEX + defender.finalStats.AC + (defender.bonuses.get("dodge") || 0);
        ...
        let bonus: number = this.entity.bonuses.get(this.weapon?.damageType || "") || 0;
        let penalty: number = defender.resistances.get(this.weapon?.damageType || "") || 0;

        let baseDamage = this.weapon?.damage || 5;

        if (!this.weapon?.ranged) baseDamage += (this.entity.finalStats.STR - 10) / 2;
        ...
        if (this.weapon?.attackForce) {
            ...
            let knockbackPower =
                (this.weapon?.attackForce || 0) * (this.entity.finalStats.STR / (defender.finalStats.STR || 1));
        ...
        if (defender.entityType !== "BOSS" && !defender.isImmune() && this.weapon?.crowdControlEffect) {
        ...
        for (let effect of this.weapon.statusEffects || []) {
            import("../statusEffects/statusEffectFactory").then(({ statusEffectFactory }) => {
