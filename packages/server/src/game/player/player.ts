...
export class Player extends Entity {
    ...
    initDocument(playerDocument: IPlayer) {
        ...
        const appearanceItems = ["hat", "frontextra", "head", "backhair", "hair", "top", "bottom", "backextra"];
        appearanceItems.forEach(item => {
            if (playerDocument.appearance[item as keyof IPlayer["appearance"]]) {
                this.appearance.set(item, playerDocument.appearance[item as keyof IPlayer["appearance"]]);
            }
        });
    ...
    initInventory(playerDocument: IPlayer) {
        ...
        for (let r = 0; r < this.inventory.rows; r++) {
            for (let c = 0; c < this.inventory.cols; c++) {
                const dbItem = playerDocument.inventoryGrid[r * this.inventory.cols + c];
                ...
                const item: InventoryItem = new InventoryItem(dbItem.itemId, dbItem.quantity);
    ...
    handleInventoryChange(key: string, message: any) {
        ...
        if (
            ...
            validateDestination &&
            message.key &&
            ["weapon", "offhand", "helmet", "chest", "legs", "boots"].includes(message.key)
        ) {
